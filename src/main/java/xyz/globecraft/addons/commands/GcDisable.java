...
public class GcDisable implements CommandExecutor {
    private final AddonsPlugin addons;

    public GcDisable(AddonsPlugin addons) {
        this.addons = addons;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        for(AddonInstance instance : this.addons.getAddons()) {
            if(instance.getName().equalsIgnoreCase(String.join(" ", Arrays.asList(args)))) {
                if(!instance.loaded()) {
                    s.append(ChatColor.RED).append("Addon already disabled!");
                ...
                s.append(ChatColor.RED).append("disabled.");
                sender.sendMessage(s.toString());
                return true;
            }
        }
